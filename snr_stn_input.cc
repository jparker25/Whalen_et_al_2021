...
// dynamical variables
	...
	Neuron();
	void init(double,double,double,double,double,double,double,double,double,double,double); // seems like this is never used //added a double for gstn_static
...
class Population
{
	...
	Population(int n,double gca0,double gca1,double gcal0,double gcal1,double gl0,double gl1,double gnap0,double gnap1,
		...
		bool gauss_rand, int seed,double gtonstnstatic)
	{
		connect = conn;
		...
		*poutpoint<<"SNr Neuron Tonic Exc. Drives:"<<std::endl;
		...
		for(int i=0;i<size;i++)
		{
			...
			net[i].gton_stn_static= c_dend*gtonstnstatic;
...
void Neuron::init(double gca,double gcal,double glk,double gnap,double gcan,double gchr2,double garch,double gsk,double ghcn ,double gtrpc3,double gstonstatic)
{
	gCa=gca;
	gCaL=gcal;
	g_L=glk;
...
void Neuron::step(double dt,double drive,double fcan,double fsynCa, double irr, double iapplied, double ext_inh, double ext_exc) // iapplied is built-in applied current, ext_inh is external inh. stim by TW
{
	...
	if(!QIF) {
		...
		//gstn_ton_static = gstntonstatic*c_dend;
		Inoise = 0.0;
		// Adjusted to do NO GPe input
		gEXCDEND =0;
		gDENDSYN =0;
		fDENDSYN =0;
		gSOMSYN =0;
		...
		v+=dv*dt;
		dv_dend=(-(gsyn_dend)*(v_dend-EGABA_dend) -gDENDSYN*(v_dend-EGABA_dend) -ISOMdend-ITRPC3_dend -gEXCDEND*(v_dend-Eexc) - (gton_stn+gton_stn_static)*(v_dend-Eexc) +ton_dend_dr)/c_dend;
		v_dend+=dv_dend*dt;
...
class PoissonFunctionNeuron: public StimulusNeuron // Poisson neuron with lambda determined by arbitrary function of time
{
	...
	double refrac;
	double last_stim;
	std::function<double(const double&)> rate_func;
	int cellnum; // index in total GPe population
	std::ofstream* goutpoint;
...
int main(int argc,char** argv)
{
	...
	for(int i=1;i<argc;i++)
	{
		...
		else if(strcmp(argv[i],"-gton_stn_static")==0) gstn_static=atof(argv[++i]); // need for STN static
		else if(strcmp(argv[i],"-wgstn_dyn")==0) Wgstn=atof(argv[++i]); // need for STN dynamics
		else if(strcmp(argv[i],"-stn_delay")==0) stn_delay=atof(argv[++i]); // delay for stn dynamics
		else fname=argv[i];
	}

	int n_timestep = 0;
	std::vector<int> delta_value;
	double n_timestep_delay = (stn_delay*1000)/dt;
	...
	Population pop(size,gca[0],gca[1],gcal[0],gcal[1],gleak[0],gleak[1],gnap[0],gnap[1],gcan[0],gcan[1],gchr2[0],gchr2[1],garch[0],garch[1],snr_wght[0],snr_wght[1],gpe_wght[0],gpe_wght[1],stn_wght[0],stn_wght[1],stimnum,gsk[0],gsk[1],ghcn[0],ghcn[1],gtrpc3[0],gtrpc3[1],gsd,gkcc2,gkcc2_dend, n_ext_gpe, n_ext_stn, conn, conn_gpe, conn_stn, &pout, gauss_rand, seed, gstn_static );
	...
	// Extra stuff for STN implementation

	ofstream stn_data;
	stn_data.open("stn_data_ON.txt");
	stn_data << "# this file contains data pertaining to STN implementation" << endl;

	ifstream delta_multiplier("delta_multiplier.txt");
	if (delta_multiplier.good()){
		int current_number = 0;
		while(delta_multiplier >> current_number){
			delta_value.push_back(current_number);
		}
		delta_multiplier.close();
		ofstream test_delta("test_delta.txt");
		for(int kk = 0; kk < delta_value.size(); kk++){
			test_delta << delta_value[kk] << " ";
		}
		test_delta << endl;
		test_delta.close();
	...
	for(double t=0;t<=T;t+=dt)
	{
		...
		/* while loop for initial transient of delay time for stn dynamics */
		double delay_t = -1*stn_delay*1000;
		while(n_timestep < n_timestep_delay){
			stn_data << delay_t << " ";
			for(int i = 0; i<size;i++){
				stn_data << pop.net[i].v << " "<< pop.net[i].v_dend << " " << pop.net[i].gton_stn/c_dend << " " << pop.net[i].gton_stn_static/c_dend << " " << Wgstn << " ";
				if (n_timestep > 0){
					pop.net[i].gton_stn = ((pop.net[i].gton_stn/c_dend) + (delta_value[n_timestep]*Wgstn-(pop.net[i].gton_stn/c_dend)/3)*dt)*c_dend;
				}
			}
			stn_data << endl;
			n_timestep++;
		...
		/* actual update for stn dynamics during simulation */
		stn_data << t << " ";
		for(int i = 0; i<size;i++){
			stn_data << pop.net[i].v << " "<< pop.net[i].v_dend << " " << pop.net[i].gton_stn/c_dend << " " << pop.net[i].gton_stn_static/c_dend << " " << Wgstn << " ";
